 ...
 *        list is unavailable)
 *       「模型用量」     same-day (Beijing) per-model token usage (best effort; monitor aggregates with a lag)
	...
	factory: (require) => {
		...
		var DICT = {
			zh: {
				...
				apiThresholdTypeTokens: "今日 Token",
				apiThresholdTypeBalance: "累计消费 ¥",
			...
			en: {
				...
				sectionCoding: "Coding Plan",
				sectionUsage: "Model usage · today",
				...
				apiThresholdTypeTokens: "Tokens today",
				apiThresholdTypeBalance: "Total spend ¥",
		...
		function readSettings() {
			...
			// apiThreshold* — manual API-mode consumption alert (tokens today or
		...
		function SettingsPanel(props) {
			...
			// API 消耗预警：手动设阈值（今日 Token 或累计消费 ¥），
			// 达到后黄点告警；勾选“停止调用”则宿主端把调用地址指向拒绝端口。
			var apiThresholdBlock = [
				React.createElement("div", { key: "thr-title", className: "dzb-setting-title" }, t("apiThresholdTitle")),
				React.createElement("div", { key: "thr-type", className: "dzb-setting-row" },
					React.createElement("span", { className: "dzb-setting-name" }, t("apiThresholdType")),
					React.createElement("select", {
						value: settings.apiThresholdType,
						onChange: function (e) {
							onChange(Object.assign({}, settings, { apiThresholdType: e.target.value }));
						}
					},
				...
				React.createElement("div", { key: "thr-value", className: "dzb-setting-row" },
					...
					React.createElement("input", {
						...
						onChange: function (e) {
							...
							onChange(Object.assign({}, settings, { apiThresholdValue: e.target.value === "" || !isFinite(v) || v <= 0 ? null : v }));
						}
					})),
				React.createElement("div", { key: "thr-stop", className: "dzb-setting-row" },
					React.createElement("span", { className: "dzb-setting-name" }, t("apiThresholdStop")),
					React.createElement("input", {
						type: "checkbox",
						checked: settings.apiThresholdStop,
						onChange: function (e) {
							onChange(Object.assign({}, settings, { apiThresholdStop: e.target.checked }));
						}
					}))
			];
		...
		function apply(ctx) {
			...
			function Panel(props) {
				...
				["ok", "warn", "error"].forEach(function (level) {
					...
					if (apiView && apiVisible) {
						if (apiView.packs) apiView.packs.forEach(function (p) { scan(p.percent); });
						apiView.entries.forEach(function (e) { scan(e.percent); });
					}
					if (codingView && codingVisible) {
						["rolling", "weekly", "monthly"].forEach(function (k) {
							if (codingView.windows[k]) scan(codingView.windows[k].percent);
						});
					}
				});
				// Errors on the active mode's data = can't determine/verify → red.
				if (apiError && apiVisible) worst = "error";
				if (codingError && codingVisible) worst = "error";

				// API 消耗预警（仅 API Key 模式）：今日 Token 或累计消费 ¥ 达到
				// 手动阈值 → 至少黄点；勾选停止调用时由下方 effect 通知宿主端。
				var apiThresholdConsumed = null;
				if (callMode === "api" && settings.apiThresholdValue !== null && settings.apiThresholdValue > 0 && apiView && !apiError) {
					if (settings.apiThresholdType === "balance") {
						if (apiView.cash && typeof apiView.cash.spend === "number") apiThresholdConsumed = apiView.cash.spend;
					} else if (apiView.detail && apiView.detail.length > 0) {
						var consumedTokens = 0;
						for (var di = 0; di < apiView.detail.length; di++) consumedTokens += apiView.detail[di].tokens;
				...
				if (apiThresholdExceeded && worst === "ok" || apiThresholdExceeded && worst === "muted") worst = "warn";

				// 状态点收敛：数据加载正常且无告警 → 绿色（可用）。
				if (worst === "muted" && ((apiVisible && apiView && !apiError) || (codingVisible && codingView && !codingError))) worst = "ok";
				...
				if (settingsOpen) {
					...
					if (apiVisible) {
						...
						if (apiError) {
							...
							if (settings.showModelUsage) {
								// Always visible so the section state is discoverable;
								// the monitor backfills late, so empty ≠ unused.
								apiChildren.push(React.createElement("div", { key: "usage-divider", className: "dzb-divider" }));
								apiChildren.push(React.createElement("div", { key: "usage-title", className: "dzb-section-title" }, "模型用量 · 当天"));
								if (apiView.detail && apiView.detail.length > 0) {
									for (var m = 0; m < apiView.detail.length; m++) {
										var model = apiView.detail[m];
										apiChildren.push(React.createElement("div", { key: model.name, className: "dzb-row" },
									...
									apiChildren.push(React.createElement("div", { key: "usage-empty", className: "dzb-hint" }, "暂无入账（智谱统计有延迟）"));
								}
							}
							if (apiThresholdExceeded) {
								apiChildren.push(React.createElement("div", { key: "guard", className: "dzb-hint" },
									t("guardNotice") + (settings.apiThresholdStop ? "" : "（未勾选停止调用）")));
							}
						}
						body.push(React.createElement("div", { key: "api", className: "dzb-section" },
							SectionTitle(showPacks ? t("sectionPacks") : (apiView.entries.length > 0 ? "账号额度" : t("sectionApi")), apiView ? apiView.plan : null, foldOf("api")),
