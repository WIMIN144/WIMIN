...
/**
 * Best-effort same-day (Beijing) per-model usage; null when unavailable.
 * The monitor aggregates with a lag, so recent calls surface late
...
async function fetchUsageDetail(baseUrl, key, schemeState) {
	...
	const start = Date.UTC(beijing[0], beijing[1] - 1, beijing[2]) - 8 * 60 * 60 * 1000;
